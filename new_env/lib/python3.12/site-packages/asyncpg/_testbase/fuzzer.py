...
class TCPFuzzingProxy:
    ...
    async def _wait(self, work):
        ...
        try:
            await asyncio.wait(
                [work_task, stop_event_task], return_when=asyncio.FIRST_COMPLETED
            )
    ...
    def start(self):
        ...
        self.thread = threading.Thread(target=self._start_thread, args=(started,))
        self.thread.start()
    ...
    async def listen(self):
        while True:
            try:
                client_sock, _ = await self._wait(self.loop.sock_accept(self.sock))
...
class Connection:
    ...
    async def handle(self):
        self.proxy_to_backend_task = asyncio.ensure_future(self.proxy_to_backend())

        self.proxy_from_backend_task = asyncio.ensure_future(self.proxy_from_backend())
    ...
    async def _read(self, sock, n):
        ...
        try:
            await asyncio.wait(
                [read_task, conn_event_task], return_when=asyncio.FIRST_COMPLETED
            )
    ...
    async def _write(self, sock, data):
        ...
        try:
            await asyncio.wait(
                [write_task, conn_event_task], return_when=asyncio.FIRST_COMPLETED
            )
